...
class WRFDataTS:
    def __init__(self, folderpath, verbose = False, datafields = ["all"]):
    ...
    def loadDataSets(self,verbose=2):
        ...
        for fn in self.fullfilenames:
            wrf = WRFData(fn,verbose,self.datafields)
            wrfs.append(wrf)
        # for END
        return wrfs
    # def END
    ...
    def produceTimeSeries(self):
        ...
        if "rainnx" in self.datafields:
            rainph = []
            for i in range(count):
                for j in range(self.wrfs[i].getDPCount()):
                    rainph.append(self.wrfs[i].rainnx[j])
            self.rainph = np.array(rainph)
            self.rainph_unit = self.wrfs[0].rainnx_unit if count > 0 else None

        if "snownx" in self.datafields:
            snowph = []
            for i in range(count):
                for j in range(self.wrfs[i].getDPCount()):
                    snowph.append(self.wrfs[i].snownx[j])
            self.snowph = np.array(snowph)
            self.snowph_unit = self.wrfs[0].snownx_unit if count > 0 else None
    # def END
